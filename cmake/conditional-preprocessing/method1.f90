...
! Method 1 : C-like Preprocessing. Works with gfortran, ifort and PGI
! ======================================================================
! This file contains simple examples of conditional compilation. The compilation preprocessor is usually a C preprocessor. 
! Therefore, the synthax used to formulated the conditional statements is in C language and must be flagged by a # (comment).
! Because it involve C language mixed with Fortran language, one single compiler is not enough. For this, the preprocessor is 
! is a C compilor, running all the flagged tasks (with #) and writing a modified Fortran Script, which is compiled as usual by 
...
! about these preprocessor macros with the command "gfortran -cpp -E -dM maing.f90". Some are common between all compilers, but 
! some are not (for example, __linux macro exist with ifort and pgfortran, but not on gfortran.)

! /!\ # must be the first character of the line. NO BREAK!!!!!!
...
#elif defined __linux || defined __GFORTRAN__
